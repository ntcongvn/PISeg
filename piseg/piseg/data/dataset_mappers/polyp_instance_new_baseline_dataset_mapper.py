...
__all__ = ["POLYPInstanceNewBaselineDatasetMapper"]


def center_crop(image,size=None):
  if size is None:
    if image.shape[0]==image.shape[1]:                                          #h=w
      return image
    return center_crop(image,size=np.min(image.shape))                          #Crop min size of image edges
  h,w,_ = image.shape
  startx = (w-size)//2
  starty = (h-size)//2    
...
class POLYPInstanceNewBaselineDatasetMapper:
    """
    ...
    def __init__(
        ...
        *,
        ...
        logging.getLogger(__name__).info(
            "[POLYPInstanceNewBaselineDatasetMapper] Full TransformGens used in training: {}".format(str(self.tfm_gens))
        )
